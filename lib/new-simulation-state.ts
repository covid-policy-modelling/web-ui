...
import {Region, TopLevelRegion} from '../pages/api/regions'
...
export function createFormBody(state: NewSimulationState): NewSimulationConfig {
  return {
    ...
    label: state.label,
    r0: state.r0,
    customCalibrationDate: state.customCalibrationDate || undefined,
...
type Action =
  ...
  | {
      type: 'SET_CUSTOM_CALIBRATION_DATE'
      customCalibrationDate: ISODate | undefined
    }
  | {
      type: 'SET_REGION'
...
export function initializeSimulationState(
  ...
  return {
    ...initialState,
    r0: undefined,
    customCalibrationDate: undefined,
...
export function reducer(
  ...
  switch (action.type) {
    ...
    case 'SET_CUSTOM_CALIBRATION_DATE':
      return {...state, customCalibrationDate: action.customCalibrationDate}
...
export function compareIntensities(
  i1: input.Intensity | undefined,
  i2: input.Intensity | undefined
): number {
  if (i1 === i2) {
    return 0
  }
...
// Exported for testing
export function compareRawInterventions(
  i1: RawIntervention,
  i2: RawIntervention
) {
  const dateCompare = i1.startDate.localeCompare(i2.startDate)
  // repeal is greater than enact
  const repealCompare = i1.isRepeal !== i2.isRepeal ? (i1.isRepeal ? 1 : -1) : 0
  const intensityCompare =
    compareIntensities(i1.intensity, i2.intensity) * (i1.isRepeal ? -1 : 1)
  return dateCompare || repealCompare || intensityCompare
}

// Assumption is that each intervention will be applied and then possibly
// repealed. There will not be multiple applications of the same intervention.
function autoFillParameters(
  ...
  const rawInterventions: RawIntervention[] = []

  function addIntervention(
    name: string,
    type: StrategyKey,
    intensity: input.Intensity,
    repealIntensity: input.Intensity | undefined
  ) {
    ...
    if (namedInterventions?.dateEnded) {
      rawInterventions.push({
        type,
        intensity: repealIntensity,
        startDate: namedInterventions.dateEnded,
        isRepeal: true
      })
    }
  }
  ...
  let interventionPeriods: InterventionPeriod[] = rawInterventions
    // Sort by start date,
    // then by repeal state
    // then by intensity (reverse intensity for repeals)
    .sort(compareRawInterventions)

    // create initial interventions set
    .map<InterventionPeriod>(raw => ({
  ...
  interventionPeriods = JSON.parse(JSON.stringify(interventionPeriods))
