...
export function parse(gbCasesJSON, gbNationCasesJSON) {
  ...
  const gbNationCaseRecords = parseSubregionData(gbNationCasesJSON)
...
function parseSubregionData(gbNationCasesJSON) {
  //-----------------------------------------------------------
  ...
  const gbCaseRecords = parsed.body
    ...
    .map(row => {
      const regionID = 'GB'
      const subregionID = countryMap[row.areaCode] || row.areaCode
      // Date is already a string with the format YYYY-MM-DD
      // but will leave the code below as is
      const dateSQL = DateTime.fromFormat(
        row.date.toString(),
